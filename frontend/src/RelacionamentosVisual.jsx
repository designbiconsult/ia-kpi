...
// Exemplo de tabelas com colunas (substitua pelo seu backend!)
const tabelasFake = [
...
export default function RelacionamentosKonva() {
  ...
  // Handlers de drag das tabelas
  const handleDragStart = (idx, e) => {
    dragOffset.current = {
      x: e.evt.layerX - nodes[idx].x,
      y: e.evt.layerY - nodes[idx].y,
    };
    setNodes((nds) =>
      nds.map((n, i) =>
  ...
  const handleDragMove = (idx, e) => {
    const x = Math.max(0, Math.min(CANVAS_W - nodes[idx].width, e.evt.layerX - dragOffset.current.x));
    const y = Math.max(0, Math.min(CANVAS_H - nodes[idx].height, e.evt.layerY - dragOffset.current.y));
    setNodes((nds) =>
      nds.map((n, i) =>
        i === idx ? { ...n, x, y } : n
      )
  ...
  const handleResizeMove = (e) => {
    ...
    // Não deixa passar da borda direita do canvas
